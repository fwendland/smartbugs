...
def add_vul(contract, tool, vulnerability, lines):
    ...
    if original_vulnerability in vulnerability_mapping:
        ...
        print("Vulnerability is not part of vulnerability_mapping:", original_vulnerability, "Tool:", tool)
    ...
    for contract in os.listdir(path_tool_result):
        ...
        with open(path_result, 'r', encoding='utf-8') as fd:
            ...
            if tool == 'mythril':
                ...
                analysis = data['analysis']
                for result in analysis:
                    vulnerability = result['vuln_type'].strip()
                    add_vul(contract, tool, vulnerability, [int(result['line_number'])])
            elif tool == 'ccc':
                for analysis in data['analysis']:
                    if analysis['errors'] is not None:
                        for result in analysis['errors']:
                            vulnerability = result['vulnerability'].strip()
                            add_vul(contract, tool, vulnerability, [int(result['location']['region']['start_line'])])
            else:
